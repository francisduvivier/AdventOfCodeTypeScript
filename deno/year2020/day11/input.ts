export const rInput = getR().split('\n').map(row => row.split('') as ('#' | '.' | 'L')[]);
export const tInput = getT().split('\n').map(row => row.split('') as ('#' | '.' | 'L')[]);
...
function getR() {
    return `LLLLLLLLLLLLLLL.LL.L.LLLLLLLLLLLLLLL.LLLL.LLLL.LLLLLLLLL.LLLLLLLLLLLLLLLLLLLLLL.LLLLL.LLLLLLLLL
