import {input, testInput1, testInput2, testInput3, testInput5, testSol1, testSol2, testSol5} from "./input.ts";
import { assertNotEquals, assertEquals } from "https://deno.land/std@0.80.0/testing/asserts.ts";
...
function doReactionRec(r: Reaction, outPutNeeded: number, ioMap: Map<CHEM, Reaction>, chemLevels: Map<CHEM, number>) {
    assertEquals(chemLevels.get(r.output[1]) || 0, 0);
    ...
    r.inputs.forEach((timesInput, chem) => {
        ...
        if (needLeft > 0) {
            ...
            assertNotEquals(r1, undefined, chem);
...
assertEquals(part1(testInput1), testSol1);

console.log('doing testInput2');
assertEquals(part1(testInput2), testSol2);
console.log('doing testInput5');
assertEquals(part1(testInput5), testSol5);
console.log('doing input');
assertEquals(part1(input), 469536);
...
assertEquals(part2(1000000000000, testInput3), 82892753);
console.log('done test 3');
assertEquals(part2(1000000000000, testInput5), 460664);
