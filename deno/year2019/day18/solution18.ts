...
import { DIRS, getNewPos } from "../util/GridRobot.ts";
...
import { isLetter, isLowerletter, isUpperletter, keysToSortedString, sortString } from "../util/Strings.ts";
...
assertEquals(origArray, [1, 'insert', 2, 3, 4, 5, 6].map(String));
...
function calcLeastMoves(inputString = input, splitTask?: boolean): State | undefined {
    ...
    function isOkState(newState: State) {
        ...
        if (bestEquivalentState === undefined) {
            // assertEquals(unExploredStatesWDirList.indexOf(newState.uuid), -1, newState.uuid);
            return true;
        }
    ...
    function addUnexploredOkState(newState: State) {
        ...
        assertNotEquals(bestStepsForState.get(newState.uuid), unExploredStatesWDirList, newState.uuid);
    ...
    function getNextUnexplored(): State {
        ...
        assertNotEquals(newState, undefined, '' + newStateKey + unExploredStatesWDirList.length);
...
export class State {
    ...
    constructor(public prevState: State | undefined, readonly p: P[], readonly moverIndex: number | undefined, grid: Grid<string>) {
        ...
        if (moverIndex !== undefined) {
            ...
            assertNotEquals(newPosVal, undefined);
...
assertEquals(solutions[solutions.length - 1], testSol0);
solutionStates.push(calcLeastMoves(testInput1));
logAndPushSolution(solutionStates[solutions.length]!.steps, solutions);
assertEquals(solutions[solutions.length - 1], testSol1);
solutionStates.push(calcLeastMoves(testInput2));
logAndPushSolution(solutionStates[solutions.length]!.steps, solutions);
assertEquals(solutions[solutions.length - 1], testSol2);
solutionStates.push(calcLeastMoves());
logAndPushSolution(solutionStates[solutions.length]!.steps, solutions);
assertEquals(solutions[solutions.length - 1], 4620);

solutionStates.push(calcLeastMoves(testInput6, true));
logAndPushSolution(solutionStates[solutions.length]!.steps, solutions);
assertEquals(solutions[solutions.length - 1], testSol6);
solutionStates.push(calcLeastMoves(testInput5, true));
logAndPushSolution(solutionStates[solutions.length]!.steps, solutions);
// assertEquals(solutions[solutions.length - 1], testSol5);
testSol5;
solutionStates.push(calcLeastMoves(testInput4, true));
logAndPushSolution(solutionStates[solutions.length]!.steps, solutions);
assertEquals(solutions[solutions.length - 1], testSol4);

solutionStates.push(calcLeastMoves(input, true));
logAndPushSolution(solutionStates[solutions.length]!.steps, solutions);
assertEquals(solutions[solutions.length - 1], 1564);
