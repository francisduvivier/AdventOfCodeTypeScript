...
function processOpCode(pos, mem) {
    let instuction = `${mem[pos]}`;
    const opCode = Number(instuction.slice(instuction.length - 2));
    const modesString = instuction.slice(0, instuction.length - 2);
    let nbArgs = 0;
    let operation = () => {
    };
    switch (opCode) {
...
let noun = 12;
let verb = 2;
const part1Result = runIntCodeProgram([...input], noun, verb);
